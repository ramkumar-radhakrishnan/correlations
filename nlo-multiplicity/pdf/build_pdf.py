#!/usr/bin/env python3
"""Render the analysis markdown files into a single typeset PDF.
...
FreeSans is used for the body (it is the only installed sans family with a
complete regular/bold/oblique set) and DejaVu Sans Mono for code blocks (best
box-drawing and math coverage).  Run from anywhere:

    python3 pdf/build_pdf.py [output.pdf]
"""
...
def render(md, S, avail, heading_sink=None):
    ...
    while i < n:
        ...
        if m:
            ...
            txt = m.group(2).strip()
            if heading_sink is not None and lvl <= 2:
...
class Doc(BaseDocTemplate):
    ...
    def decorate(self, canv, doc):
        ...
        canv.drawString(self.leftMargin, self.pagesize[1] - 12 * mm,
                        "Gluon multiplicity moments from the NLO CGC evolution operator")
        ...
        canv.drawString(self.leftMargin, self.bottomMargin - 8.5 * mm,
                        "arXiv:2607.18373 follow-up analysis")
        canv.restoreState()
...
def main():
    ...
    out = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
        root, "NLO_CGC_multiplicity_analysis.pdf")
    ...
    story += [
        Spacer(1, 34 * mm),
        Paragraph("Gluon Multiplicity Moments and<br/>Particle-Number Fluctuations "
                  "at Order g⁴", S["title"]),
        Spacer(1, 4 * mm),
        Paragraph("A formalism, feasibility and publication analysis of the programme "
                  "announced in", S["subtitle"]),
        Paragraph("<i>Soft Gluon Wave Function and Evolution Operator in the CGC "
                  "at Next-to-Leading Order</i>", S["subtitle"]),
        Paragraph('<a href="https://arxiv.org/abs/2607.18373" color="#17559c">'
                  "arXiv:2607.18373</a> &nbsp;·&nbsp; R. Radhakrishnan", S["subtitle"]),
        Spacer(1, 12 * mm),
    ...
    for line in [
        ...
        "(2) Because ⟨aa⟩<sub>c</sub> is O(g²) while "
        "⟨a<super>†</super>a⟩<sub>c</sub> is O(g⁴), the leading "
        "non-Poissonian effect is displacement–squeezing interference, and C₂ is "
        "complete at O(g⁴) with the existing operator. "
        ...
        "at the same order. And the classical and quantum contributions cannot be separated by "
        "power counting.",
        ...
        story.append(Paragraph(line, ParagraphStyle(
            "cv", parent=S["body"], fontSize=8.7, leading=13,
            leftIndent=10 * mm, rightIndent=10 * mm, spaceAfter=7)))

    story += [
        Spacer(1, 14 * mm),
        Paragraph("Prepared with Claude Code. Parts I–IX were first drafted without access "
                  "to the paper and were subsequently reconciled against v2; the first section "
                  "records exactly what changed.", S["tiny"]),
    ]

    # ---- render bodies, collecting headings for the contents page
    bodies, toc = [], []
    for fn, label in order:
        path = os.path.join(root, "docs", fn)
        with open(path, encoding="utf-8") as fh:
            md = fh.read()
        marks = []
        flow = render(md, S, avail, heading_sink=marks)
        toc.append((label, [t for lvl, t in marks if lvl == 2][:9]))
        bodies.append(flow)

    story.append(PageBreak())
    story.append(Paragraph("Contents", S["h1"]))
    story.append(HRFlowable(width="100%", thickness=0.7, color=RULE, spaceAfter=9))
    for label, subs in toc:
        story.append(Paragraph("<b>%s</b>" % html.escape(label), S["toc"]))
        for s in subs:
            story.append(Paragraph(
                "&nbsp;&nbsp;&nbsp;&nbsp;<font color='#5a6472'>%s</font>"
                % html.escape(s),
                ParagraphStyle("toc2", parent=S["toc"], fontSize=8.3, leading=12)))
